...
### 3.2.3. Composición Taxonómica

Para entender qué grupos microbianos componen las comunidades, se visualizó la abundancia relativa de los filos bacterianos más dominantes en cada muestra. Este análisis nos permite pasar de una visión abstracta de la "diversidad" a una caracterización concreta de la estructura de la comunidad.

```{r}
#| label: taxonomic-composition-plot
#| fig-cap: "Composición relativa de los filos bacterianos más abundantes. Cada barra representa una muestra individual, agrupada por su condición experimental. Los filos con menor abundancia promedio se agrupan en la categoría 'Other'."

if (safe_check("taxonomic_composition_plot")) {
  taxonomic_composition_plot
} else {
  cat("Gráfico de composición taxonómica no disponible.")
}
```
